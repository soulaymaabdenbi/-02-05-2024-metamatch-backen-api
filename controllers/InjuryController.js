...
const logoPath = path.join(__dirname, 'log.jpg');
  ...
  try {
    ...
    fs.createReadStream(req.file.path)
      ...
      .on("data", async (data) => {
        ...
        const player = await joueurModel.findOne({ name: playerName });
        if (!player) {
...
async function generateInjuryReport(injuryId) {
  try {
    const injuryData = await injury.findById(injuryId); // Renommer la variable locale
    if (!injuryData) {
      throw new Error('Injury not found');
    }

    const doc = new PDFDocument();
    const fileName = `injury_report_${injuryId}.pdf`;
    const filePath = `reports/${fileName}`; 

    // Vérifier l'existence du répertoire
    if (!fs.existsSync('reports')) {
      fs.mkdirSync('reports'); // Créer le répertoire s'il n'existe pas
    }
    ...
    doc.image(logoPath, 50, 50, { width: 50 });
     ...
     // En-tête du PDF en vert
     doc.fillColor('green').fontSize(20).text('Injury Report', { align: 'center' });
     doc.fillColor('green').fontSize(14).text(`Injury ID: ${injuryId}`, { align: 'center' });
 
    doc.moveDown();

    // Contenu du rapport
    doc.fillColor('black').fontSize(12).text(`Date: ${injuryData.date}`);
    doc.fillColor('black').fontSize(12).text(`Type: ${injuryData.type}`);
    doc.fillColor('black').fontSize(12).text(`Description: ${injuryData.description}`);
    doc.fillColor('black').fontSize(12).text(`Recovery Status: ${injuryData.recovery_status}`);
    doc.fillColor('black').fontSize(12).text(`Duration: ${injuryData.duration}`);
    
    doc.end(); // Terminer le document

    // Événement de fin de flux
    stream.on('finish', () => {
      console.log('PDF generated successfully.');
      // Vérifier l'existence du fichier après la fin du flux
