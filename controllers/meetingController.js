...
exports.getPlayers = async (req, res) => {
    try {
        const players = await User.find({ role: 'Player' }, '_id email username');
...
exports.getMeetingsByConfirmationStatusStats = (req, res) => {
    Meeting.aggregate([
      { $group: { _id: '$confirmed', count: { $sum: 1 } } },
      { $project: { confirmation_status: '$_id', count: 1, _id: 0 } }
    ])
    .then(stats => {
      
      const mappedStats = stats.map(stat => ({
        confirmation_status: stat.confirmation_status ? 'Confirmed' : 'Pending',
        count: stat.count
      }));
      res.status(200).json(mappedStats);
    })
    .catch(error => {
      console.error('Error fetching meetings by confirmation status stats:', error);
      res.status(500).json({ error: 'Could not fetch meetings by confirmation status stats' });
    });
  };
  exports.getMeetingsByMonthStats = async (req, res) => {
    try {
