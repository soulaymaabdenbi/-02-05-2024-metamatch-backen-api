...
router.get('/generateInjuryReport/:id', async (req, res) => {
    try {
        const injuryId = req.params.id;
        const filePath = await injuryControllerr.generateInjuryReport(injuryId);

        // Envoyer le fichier PDF en réponse
        res.download(filePath);
    } catch (error) {
        console.error("Error generating injury report:", error);
        res.status(500).json({ error: "Error generating injury report" });
    }
});
